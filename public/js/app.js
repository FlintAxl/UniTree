  ...
  async function fetchCategories() {
    try {
        ...
        const categoriesWithProductCount = await Promise.all(categories.map(async category => {
            ...
            return { ...category, items: productCount };
        }));
...
async function getProductCountByCategory(categoryId) {
    try {
        const response = await fetch(`/api/v1/products/count?categoryId=${categoryId}`);
...
function getCategoryImage(categoryName) {
    if (!categoryName) return 'images/category-default.jpg';
